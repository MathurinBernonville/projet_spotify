...
with DAG(
            ...
            for track in catalog.get("tracks", []):
                if all(k in track for k in ["id", "artist_id", "title", "duration_ms"]):
                    ...
                    cursor.execute(
                        """INSERT INTO dead_letter_events (original_topic, payload, error_type, error_message)
                           VALUES (%s, %s::jsonb, %s, %s)""",
                        ("minio_catalog", json.dumps(track), "schema_validation", "Champs manquants dans track"),
                    )
    ...
    def transform_catalog(validated: dict) -> dict:
        ...
        for a in data["artists"]:
            ...
            if key not in seen_artists:
                ...
                artists.append({**a, "name": name, "label": label, "genres": a.get("genres", []), "monthly_listeners": a.get("monthly_listeners", 0)})
        tracks = []
        for t in data["tracks"]:
            duration = t.get("duration_ms", 0)
            if 0 < duration < 3_600_000:
                tracks.append({**t, "title": t["title"].strip()})
        return {"artists": artists, "albums": data["albums"], "tracks": tracks}
    ...
    def load_to_postgres(transformed: dict, **context) -> dict:
        ...
        for t in transformed["tracks"]:
            cursor.execute(
                ...
                (t["id"], t["artist_id"], t.get("album_id"), t["title"], t["duration_ms"], t.get("genre"), t.get("bpm"), t.get("explicit", False)),
            )
            tracks_inserted += 1
        conn.commit()
        cursor.close()
        conn.close()
        stats = {"artists_inserted": artists_inserted, "albums_inserted": albums_inserted, "tracks_inserted": tracks_inserted}
    ...
    def notify_success(stats: dict, **context):
        ...
        Tracks inserees  : {stats.get('tracks_inserted', 0)}
        Artists inseres  : {stats.get('artists_inserted', 0)}
        """)
